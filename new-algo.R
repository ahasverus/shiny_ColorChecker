...
fls <- list.files(paste0(img_path, "test 4"), pattern = "_original\\.JPG$|_original\\.jpg", full.names = TRUE)

for (z in 1:length(fls)) {
  ...
  for (polynom in 1) {
    ...
    save.image(imCal, file = paste0(gsub("\\.JPG|\\.jpg|_original", "", fls[z]), "_polynom-", polynom, ".jpg"), quality = 0.7)
  }
}
